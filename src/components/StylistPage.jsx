    ...
    <div className='style-container' style={{minHeight:"90vh"}}>
      ...
      <div className='style-category-container'>
        {userinfo&&obj[userinfo.gender].map((item,index)=>{
            ...
            // name[0].split("/");
          return(
            <div  key={index} className="sizecalcpage-single-cloth" style={{ backgroundColor: index % 2 == 0 ? '#9298cc' : '#f6d5e0' }} onClick={()=>handleClick(obj2[userinfo.gender][index])}><img src={item.img} style={{height:"80px",width:"80px"}}></img><h2 style={{ color: 'rgb(87 56 175)', width: '140px' }}>{item.name}</h2></div>
          )
        })} 
      </div>
      </div>
